<script lang="ts">
  ...
  const allTags = Array.from(
    new Set(
      (Object.values(data.tagIndex) as string[][]).reduce(
  ...
  let searchScrollY = 0;
  const getSearchScrollY = () =>
    (searchScrollY = searchResults?.scrollTop || 0);
...
<div class="h-full flex">
  ...
  {#if $focusedText !== null}
    ...
    <div class="hidden md:flex flex-col p-2 pt-10 basis-1/5 w-1/5 h-full">
      ...
      >
        related <button
          class="float-right hover:underline"
          on:click={closeSidebar}>[X]</button
        >
      </div>
      <div
